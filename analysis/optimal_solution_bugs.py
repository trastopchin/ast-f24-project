...
def filter_invalid(results: list[dict]) -> list[dict]:
    ...
    filtered = [
        ...
        not (result["type"] == "metamorphic" and result["relation_str"] == "optimal_tolerance == optimal_tolerance")
    ]
    return filtered
...
def filter_optimal_solution(results: list[dict]) -> list[dict]:
    ...
    filtered += [
        result for result in results if 
        result["type"] == "consistency" and
        result["status_gurobi"] == "optimal" and
        result["status_cplex"] == "optimal_tolerance"
    ]
    
    # Get all metamorphic results with optimal solutions
    filtered += [
        result for result in results if
        result["type"] == "metamorphic" and
        (result["relation_str"].endswith("optimal == optimal") or
        result["relation_str"].endswith("optimal_tolerance == optimal_tolerance"))
    ]
...
if __name__ == "__main__":
    ...
    pivot_table = df.pivot_table(index="bug", columns="type", aggfunc="size", fill_value=0)
    ...
    plt.savefig("distribution_of_optimal_solution_results_by_bug_and_type.pdf", format="pdf")
    # plt.show()
    plt.clf()
